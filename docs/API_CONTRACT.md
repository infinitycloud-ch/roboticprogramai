...
### Usage
```bash
python3 /home/panda/robotics/robotics_env/adapters/mono_robot_goto.py --target_x <float> --target_y <float> --dist_stop <float> [--robot {go2,spot,h1,anymal_c,g1}]
```
...
| `--robot` | string | non | go2 | Profil robot : `go2`, `spot`, `h1`, `anymal_c`, `g1`. Modifie les defaults ci-dessus. |

### Profils Robot (v2.9)

Le flag `--robot` charge un jeu de defaults adapté au robot cible (5 robots supportés) :

| Paramètre | Go2 (défaut) | Spot | H1 | ANYmal-C | G1 |
|-----------|-------------|------|----|----------|----|
| Type | Quadrupède 12kg | Quadrupède 32kg | Humanoïde 47kg | Quadrupède 50kg | Humanoïde 35kg |
| `vx` | 0.30 m/s | 0.40 m/s | 0.10 m/s | 0.20 m/s | 0.15 m/s |
| `pulse_s` | 2.5s | 2.0s | 1.0s | 2.0s | 1.5s |
| `stand_s` | 2.0s | 2.0s | 4.0s | 2.5s | 3.5s |
| `kp` | 3.0 | 2.0 | 1.0 | 2.0 | 1.2 |
| `wz_max` | 1.0 rad/s | 1.0 rad/s | 0.4 rad/s | 0.8 rad/s | 0.5 rad/s |
| `tol_deg` | 10° | 10° | 20° | 12° | 18° |
| `max_cycles` | 20 | 40 | 50 | 30 | 40 |
| `timeout_s` | 120s | 240s | 300s | 180s | 240s |
| `stop_and_turn` | non | **oui** | **oui** | non | **oui** |

**Design rationale** :
- **Humanoïdes (H1, G1)** : gains faibles (kp ≤ 1.2), pulses courts, stands longs — bipèdes fragiles, stabilisation longue
- **Quadrupèdes lourds (Spot, ANYmal-C)** : gains modérés, vitesse réduite vs Go2 — inertie + masse
- **Go2** : gains agressifs — léger (12kg), agile, testé en production

**Note** : Les flags explicites sur la CLI écrasent toujours le profil. Exemple :
```bash
# Utiliser profil H1 mais avec vx personnalisé
python3 mono_robot_goto.py --target_x 5 --target_y 0 --dist_stop 0.5 --robot h1 --vx 0.15
```
